...
public class showItemDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getSupportFragmentManager().findFragmentByTag("fragMessage")!=null) {

            Fragment Fragment = getSupportFragmentManager().findFragmentByTag("fragMessage");
            getSupportFragmentManager().beginTransaction().remove(Fragment).commit();
            getSupportFragmentManager().beginTransaction().remove(Fragment).commit();

            System.out.println(getSupportFragmentManager().findFragmentByTag("fragMessage"));
        }
        ...
        if(FLAT_FOR_ItemsDetail.compareTo("Item_Pool") ==0 ){
            setUpForUpload();
        }else if(FLAT_FOR_ItemsDetail.compareTo("Item_profile")==0 ){
            setUpForOwner();
        }else{
            setUpForView();
        }
    ...
    private void ChooseItemsToSend(){
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(showItemDetailActivity.this);
        ...
        //System.out.println("owner items "+theCurrentLoginUser.getOwnsItems());
        ...
        items_spinner.setAdapter(new ArrayAdapter<>(showItemDetailActivity.this,
        ...
        if(setProcessingListForStarter(event_id, loginUser, oppositeUser,  loginUserItem, chosenItem)){
            ...
            if(setPendingListForOpposite(event_id, loginUser, oppositeUser,  loginUserItem, chosenItem)){
                ...
                if(setAllEventGroupValue(event_id, loginUser, oppositeUser,  loginUserItem, chosenItem)){
                    SentRequestMessage();
                    showItemDetailActivity.super.onBackPressed();
                    finish();
                }
            }
        }
    ...
    private boolean setAllEventGroupValue(String event_id, User loginUser, User oppositeUser,  Items loginUserItem, Items chosenItem){
        ...
        realTimeDatabaseForAllEventRecord.child(event_id).child("participants").child(loginUser.getUserID()).setValue(new Participant(loginUser.getEmail(), loginUser.getPassword(), loginUser.getUserName(),
                loginUser.getPhoneNum(), loginUser.getBirthday(), loginUser.getUserID(), loginUser.getProfileImagePath(), "Accepted", loginUser.getRating()));
        ...
        realTimeDatabaseForAllEventRecord.child(event_id).child("participants").child(oppositeUser.getUserID()).setValue(new Participant(oppositeUser.getEmail(), oppositeUser.getPassword(), oppositeUser.getUserName(),
                oppositeUser.getPhoneNum(), oppositeUser.getBirthday(), oppositeUser.getUserID(), oppositeUser.getProfileImagePath(), "Waiting", loginUser.getRating()));
    ...
    private void setItemInformation(Items gottenItem){
        if(gottenItem.getPathForImagesPictures()!=null) {
            ...
            if(gottenItem.getDescription()!=null && !gottenItem.getDescription().equals("")){
                itemDescription.setText(gottenItem.getDescription());
            }else if(gottenItem.getDescription()!=null || gottenItem.getDescription().equals("")){
                itemDescription.setText("No Item description");
            }
